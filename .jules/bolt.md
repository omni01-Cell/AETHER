...
## 2025-06-03 - Optimizing Resample Floating-point Operations
**Learning:** In audio DSP loops, repeatedly invoking floating-point functions like `.floor()` and `.ceil()` creates a significant overhead.
**Action:** Replace expensive floating-point boundaries calculation (`.floor()`, `.ceil()`) with safe integer casts in resampling inner loops to eliminate unneeded overhead.
