...
## 2025-06-03 - KeyframeTrack interpolation binary search
**Learning:** Iterating linearly over keyframe timestamps during animation/property interpolation caused $O(N)$ overhead per frame/sample evaluation when tracks contained many keyframes.
**Action:** Use `slice::partition_point` to binary-search ordered keyframes in $O(\log N)$ time for fast time-segment lookup.
