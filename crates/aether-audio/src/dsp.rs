...
impl MultiTrackMixer {
    ...
    fn resample_track(
        ...
        for (ch, track_ch) in track.iter().enumerate().take(channels) {
            // Optimization (Bolt): Pre-calculate invariant inverse ratio and replace expensive floating-point floor/ceil operations with fast integer casting and logic.
            for (i, out_sample) in output[ch].iter_mut().enumerate().take(output_len) {
