...
impl MultiTrackMixer {
    ...
    fn resample_track(
        ...
        // Optimization (Bolt): Pre-calculate invariant mathematical variables (inverse ratio) and replace expensive floating-point floor/ceil with safe integer casts. Elide bounds checking by using iterators.
        let inv_ratio = from_rate as f64 / to_rate as f64;

        for (ch_idx, ch_out) in output.iter_mut().enumerate() {
            let track_ch = &track[ch_idx];
            for (i, out_sample) in ch_out.iter_mut().enumerate() {
