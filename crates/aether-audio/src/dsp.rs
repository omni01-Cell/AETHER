...
impl MultiTrackMixer {
    pub fn mix(
        ...
        for (i, track) in resampled_tracks.iter().enumerate() {
            ...
            // Optimization (Bolt): Split mutable slice to avoid mutable aliasing and use iterators to elide bounds checks
            let (mixed_l, mixed_r) = mixed.split_at_mut(1);
            let mixed_l = &mut mixed_l[0];
            let mixed_r = &mut mixed_r[0];

            // Optimization (Bolt): Loop unswitching to extract conditional outside the loop
            if track_ch == 1 {
