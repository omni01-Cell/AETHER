...
use aether_core::{
    AetherError, Asset, AssetKind, Clip, Command, CompositionGraph, Connection as GraphConnection,
    GenerationJob, GenerationStatus, Node, NodeId, NodeKind, ProjectSettings, Ref, Timeline, Track,
...
impl DbManager {
    ...
    pub fn new<P: AsRef<Path>>(dir_path: P) -> Result<Self, AetherError> {
        ...
        if !dir.exists() {
            fs::create_dir_all(dir).map_err(|e| {
                AetherError::IoError(dir.to_string_lossy().to_string(), e.to_string())
            })?;
        }
    ...
    fn initialize_schema(&self) -> Result<(), AetherError> {
        ...
        self.conn
            ...
            .map_err(|e| {
                AetherError::DatabaseError(format!("Create project_settings failed: {}", e))
            })?;
        ...
        let count_branches: i64 = self
            .conn
            .query_row("SELECT COUNT(*) FROM branches", [], |row| row.get(0))
            .map_err(|e| {
                AetherError::DatabaseError(format!("Check branches count failed: {}", e))
            })?;
        ...
        if count_branches == 0 {
            self.conn
                ...
                .map_err(|e| {
                    AetherError::DatabaseError(format!("Insert default branch failed: {}", e))
                })?;
        }
        ...
        let count: i64 = self
            .conn
            .query_row("SELECT COUNT(*) FROM project_settings", [], |row| {
                row.get(0)
            })
            .map_err(|e| {
                AetherError::DatabaseError(format!("Check settings count failed: {}", e))
            })?;
        ...
        self.conn
            ...
            .map_err(|e| {
                AetherError::DatabaseError(format!("Create state_checkpoints failed: {}", e))
            })?;
        ...
        self.conn
            ...
            .map_err(|e| {
                AetherError::DatabaseError(format!("Create generation_jobs failed: {}", e))
            })?;
        ...
        self.conn
            ...
            .map_err(|e| {
                AetherError::DatabaseError(format!("Create generation_events failed: {}", e))
            })?;
    ...
    pub fn save_asset(&self, asset: &Asset) -> Result<(), AetherError> {
        ...
        let metadata_str = serde_json::to_string(&asset.metadata).map_err(|e| {
            AetherError::DatabaseError(format!("Serialize asset metadata failed: {}", e))
        })?;
    ...
    pub fn load_assets(&self) -> Result<Vec<Asset>, AetherError> {
        let mut stmt = self
            .conn
            .prepare("SELECT ref_id, kind, path, hash, metadata FROM assets")
            .map_err(|e| {
                AetherError::DatabaseError(format!("Prepare assets query failed: {}", e))
            })?;
        ...
        for item in asset_iter {
            ...
            let metadata = serde_json::from_str(&metadata_str).map_err(|e| {
                AetherError::DatabaseError(format!("Deserialize metadata failed: {}", e))
            })?;
    ...
    pub fn load_history(
        &self,
    ) -> Result<Vec<(String, Option<String>, String, Command)>, AetherError> {
        ...
        for item in history_iter {
            let (commit_hash, parent_hash, branch, cmd_str) = item.map_err(|e| {
                AetherError::DatabaseError(format!("Read history row failed: {}", e))
            })?;

            let command: Command = serde_json::from_str(&cmd_str).map_err(|e| {
                AetherError::DatabaseError(format!("Deserialize command failed: {}", e))
            })?;
    ...
    pub fn load_branch_head(&self, name: &str) -> Result<Option<String>, AetherError> {
        ...
        let head: Option<String> = self
            ...
            .optional()
            .map_err(|e| AetherError::DatabaseError(format!("Load branch head failed: {}", e)))?;
        Ok(head)
    }
    ...
    pub fn load_all_branches(&self) -> Result<Vec<(String, String)>, AetherError> {
        ...
        let mut stmt = self
            .conn
            .prepare("SELECT name, head_commit FROM branches")
            .map_err(|e| {
                AetherError::DatabaseError(format!("Prepare branches query failed: {}", e))
            })?;

        let branch_iter = stmt
            .query_map([], |row| {
        ...
        for item in branch_iter {
            branches.push(
                item.map_err(|e| AetherError::DatabaseError(format!("Read branch failed: {}", e)))?,
            );
        }
        Ok(branches)
    }
    ...
    pub fn save_graph(&self, graph: &CompositionGraph) -> Result<(), AetherError> {
        ...
        self.conn
            ...
            .map_err(|e| {
                AetherError::DatabaseError(format!("Failed to clear connections: {}", e))
            })?;
        self.conn
            .execute("DELETE FROM graph_state", [])
            .map_err(|e| {
                AetherError::DatabaseError(format!("Failed to clear graph_state: {}", e))
            })?;
        ...
        for node in graph.nodes.values() {
            ...
            let params_str = serde_json::to_string(&node.kind).map_err(|e| {
                AetherError::DatabaseError(format!("Failed to serialize NodeKind: {}", e))
            })?;
        ...
        if let Some(out_id) = graph.output_node {
            self.conn
                ...
                .map_err(|e| {
                    AetherError::DatabaseError(format!("Failed to insert graph_state: {}", e))
                })?;
        }
    ...
    pub fn load_graph(&self) -> Result<CompositionGraph, AetherError> {
        ...
        let mut stmt = self
            .conn
            .prepare("SELECT id, params FROM nodes")
            .map_err(|e| {
                AetherError::DatabaseError(format!("Prepare nodes query failed: {}", e))
            })?;
        let node_iter = stmt
            .query_map([], |row| {
        ...
        for item in node_iter {
            ...
            let kind: NodeKind = serde_json::from_str(&params_str).map_err(|e| {
                AetherError::DatabaseError(format!("Deserialize NodeKind failed: {}", e))
            })?;
            nodes.insert(
                id as NodeId,
                Node {
        ...
        let mut stmt = self
            .conn
            .prepare("SELECT from_node, from_port, to_node, to_port FROM connections")
            .map_err(|e| {
                AetherError::DatabaseError(format!("Prepare connections query failed: {}", e))
            })?;
        let conn_iter = stmt
            .query_map([], |row| {
        ...
        for item in conn_iter {
            let conn_val = item.map_err(|e| {
                AetherError::DatabaseError(format!("Read connection row failed: {}", e))
            })?;
            connections.push(conn_val);
        }

        let output_node: Option<i64> = self
            .conn
            .query_row(
            ...
            .optional()
            .map_err(|e| AetherError::DatabaseError(format!("Get output_node failed: {}", e)))?;
    ...
    pub fn save_timeline(&self, timeline: &Timeline) -> Result<(), AetherError> {
        ...
        for (pos, track) in timeline.tracks.iter().enumerate() {
            ...
            self.conn
                ...
                .map_err(|e| {
                    AetherError::DatabaseError(format!("Failed to insert track: {}", e))
                })?;
    ...
    pub fn load_timeline(&self) -> Result<Timeline, AetherError> {
        ...
        let mut stmt = self
            .conn
            .prepare("SELECT id, name, kind FROM tracks ORDER BY position ASC")
            .map_err(|e| {
                AetherError::DatabaseError(format!("Prepare tracks query failed: {}", e))
            })?;
        ...
        for item in track_iter {
            ...
            for c_item in clip_iter {
                let (asset_ref_str, in_point, out_point, offset, trans_str) =
                    c_item.map_err(|e| {
                        AetherError::DatabaseError(format!("Read clip row failed: {}", e))
                    })?;

                let asset_ref = Ref::from_str(&asset_ref_str)?;
                let transition = match trans_str {
                    Some(s) => Some(serde_json::from_str::<TransitionKind>(&s).map_err(|e| {
                        AetherError::DatabaseError(format!("Deserialize transition failed: {}", e))
                    })?),
                    None => None,
                };
    ...
    pub fn load_keyframes(
        ...
        asset_ref: &str,
        property: &str,
    ) -> Result<Vec<(u64, f32, String)>, AetherError> {
        let mut stmt = self.conn.prepare(
        ...
        let kf_iter = stmt
            .query_map(params![asset_ref, property], |row| {
        ...
        for item in kf_iter {
            list.push(
                item.map_err(|e| {
                    AetherError::DatabaseError(format!("Read keyframe failed: {}", e))
                })?,
            );
        }
        Ok(list)
    ...
    pub fn save_generation_job(&self, job: &GenerationJob) -> Result<(), AetherError> {
        ...
        let kind_str = serde_json::to_string(&job.kind).map_err(|e| {
            AetherError::DatabaseError(format!("Serialize GenerationKind failed: {}", e))
        })?;
        let status_str = serde_json::to_string(&job.status).map_err(|e| {
            AetherError::DatabaseError(format!("Serialize GenerationStatus failed: {}", e))
        })?;
        let requested_model_str = job.requested_model.clone();
        let resolved_model_str = job
            .resolved_model
            .as_ref()
        ...
        let inputs_str = serde_json::to_string(&job.inputs).map_err(|e| {
            AetherError::DatabaseError(format!("Serialize job inputs failed: {}", e))
        })?;
        let artifacts_str = serde_json::to_string(&job.artifacts).map_err(|e| {
            AetherError::DatabaseError(format!("Serialize job artifacts failed: {}", e))
        })?;
        let error_str = job.error.clone();
        let created_at_ms = job.created_at_ms as i64;
        let updated_at_ms = job.updated_at_ms as i64;
        let options_str = serde_json::to_string(&job.options).map_err(|e| {
            AetherError::DatabaseError(format!("Serialize job options failed: {}", e))
        })?;
        ...
        self.conn
            ...
            .map_err(|e| {
                AetherError::DatabaseError(format!("Save generation job failed: {}", e))
            })?;
        ...
        self.conn
            ...
            .and_then(
                |(
                    ...
                    let kind = serde_json::from_str(&kind_str).map_err(|e| {
                        AetherError::DatabaseError(format!("Deserialize kind failed: {}", e))
                    })?;
                    let status = serde_json::from_str(&status_str).map_err(|e| {
                        AetherError::DatabaseError(format!("Deserialize status failed: {}", e))
                    })?;
                    let resolved_model = match resolved_model_str {
                        Some(s) => Some(serde_json::from_str(&s).map_err(|e| {
                            AetherError::DatabaseError(format!(
                                "Deserialize resolved_model failed: {}",
                                e
                            ))
                        })?),
                        None => None,
                    ...
                    let prompt = match prompt_str {
                        Some(s) => Some(serde_json::from_str(&s).map_err(|e| {
                            AetherError::DatabaseError(format!("Deserialize prompt failed: {}", e))
                        })?),
                        None => None,
                    };
                    let inputs = serde_json::from_str(&inputs_str).map_err(|e| {
                        AetherError::DatabaseError(format!("Deserialize inputs failed: {}", e))
                    })?;
                    let artifacts = serde_json::from_str(&artifacts_str).map_err(|e| {
                        AetherError::DatabaseError(format!("Deserialize artifacts failed: {}", e))
                    })?;
                    let options = serde_json::from_str(&options_str).map_err(|e| {
                        AetherError::DatabaseError(format!("Deserialize options failed: {}", e))
                    })?;
    ...
    pub fn load_all_generation_jobs(&self) -> Result<Vec<GenerationJob>, AetherError> {
        let mut stmt = self
            .conn
            .prepare("SELECT ref_id FROM generation_jobs")
            .map_err(|e| {
                AetherError::DatabaseError(format!("Prepare generation jobs query failed: {}", e))
            })?;
        ...
        let ref_iter = stmt
            ...
            .map_err(|e| {
                AetherError::DatabaseError(format!("Query generation jobs failed: {}", e))
            })?;

        let mut jobs = Vec::new();
        for item in ref_iter {
            let ref_id_str = item.map_err(|e| {
                AetherError::DatabaseError(format!("Read generation job row failed: {}", e))
            })?;
    ...
    pub fn delete_generation_job(&self, r: &Ref) -> Result<(), AetherError> {
        self.conn
            ...
            .map_err(|e| {
                AetherError::DatabaseError(format!("Delete generation job failed: {}", e))
            })?;
        Ok(())
    }
...
mod tests {
    use super::*;
    use aether_core::{
        BlendMode, GenerationArtifact, GenerationKind, GenerationStatus, ProjectSettings,
        ProviderModel, RefKind,
    };
    ...
    fn test_history_management() {
        ...
        let cmd1 = Command::Import {
            path: "source.mov".to_string(),
        };
        ...
        db.add_history_entry("hash2", Some("hash1"), "main", &cmd2)
            .unwrap();
    ...
    fn test_save_load_generation_jobs() {
        ...
        db.add_generation_event(&job_ref, &GenerationStatus::Ready, "Test event log")
            .unwrap();
