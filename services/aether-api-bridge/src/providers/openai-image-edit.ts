...
export async function runOpenAiImageEdit(args: {
  ...
  const params = parseParams(args.options);
    ...
    if (params.mask_path) {
      const buffer = fs.readFileSync(params.mask_path);
      const blob = new Blob([buffer], { type: mimeFromPath(params.mask_path) });
      form.append("mask", blob, path.basename(params.mask_path));
    }
